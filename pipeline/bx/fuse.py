...
def sdf_fuse(objs, voxel=0.05, fillet=0.25, name='fused', apply=True):
    """Doctrine v1 axe 1 : fusion SDF continue (voie A validée par research/tests/
    t10_sdf_nodes.py, bpy 5.0.1 headless) — remplace `voxel_fuse` quand on veut une
    masse organique unique avec congé réglable (torse+bras d'aile+racines de patte),
    plutôt qu'un simple Remesh voxel + lissage.
    Graphe GN : Mesh to SDF Grid (un par objet, le premier via l'entrée du groupe —
    modifier posé SUR lui —, les suivants via Object Info RELATIVE ; PIÈGE connu :
    ORIGINAL ignore le transform objet, RELATIVE le respecte) -> SDF Grid Boolean
    UNION (multi-input, un seul socket 'Grid' accepte tous les liens une fois
    `operation='UNION'`) -> SDF Grid Fillet (paramètre exposé = 'Iterations' seulement,
    pas de rayon direct : on convertit `fillet` (unité monde) en itérations via
    fillet/voxel) -> Grid to Mesh. `objs[0]` reçoit le modifier et devient le mesh
    final ; les autres objets sont consommés (retirés de la scène)."""
    if len(objs) < 2:
        return objs[0] if objs else None
    base, others = objs[0], objs[1:]
    ng = bpy.data.node_groups.new(f'{name}_sdf', 'GeometryNodeTree')
    ...
    boolean.operation = 'UNION'  # rend le 2e input ('Grid') multi-socket

    sdf_base = ng.nodes.new('GeometryNodeMeshToSDFGrid')
    sdf_base.inputs['Voxel Size'].default_value = voxel
    lk(n_in.outputs['Geometry'], sdf_base.inputs['Mesh'])
    lk(sdf_base.outputs['SDF Grid'], boolean.inputs['Grid'])
    ...
    for ob in others:
        ...
        lk(info.outputs['Geometry'], sdf.inputs['Mesh'])
        lk(sdf.outputs['SDF Grid'], boolean.inputs['Grid'])

    grid_out = boolean.outputs['Grid']
    if fillet:
        fil = ng.nodes.new('GeometryNodeSDFGridFillet')
        fil.inputs['Iterations'].default_value = max(1, round(fillet / max(voxel, 1e-4)))
        lk(grid_out, fil.inputs['Grid'])
        grid_out = fil.outputs['Grid']

    to_mesh = ng.nodes.new('GeometryNodeGridToMesh')
    lk(grid_out, to_mesh.inputs['Grid'])
    lk(to_mesh.outputs['Mesh'], n_out.inputs['Geometry'])

    mod = base.modifiers.new('sdf_fuse', 'NODES')
    mod.node_group = ng
    if apply:
        apply_modifiers(base)
        core.shade_smooth(base)
    base.name = name
    for ob in others:
        data = ob.data
        bpy.data.objects.remove(ob)
