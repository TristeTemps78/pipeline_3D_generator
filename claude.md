...
`forge <spec> [--fast|--clay|--sheet|--shot <id>]` (si `scene.shots` : 1 PNG par prise, cadrage auto par pièce) · `bake <spec> [--fast]` (étage high→low : shell voxel temporaire+displacement → maps/ normal/AO/courbure, ~45 s) · `clayhero <spec> [--fast]` (géométrie, cadrage macro) · `compare <spec> <ref.png> [--fast]` (réf|rendu + deltas) · `validate <spec>` (BVH sans rendu) · `sheet <spec>`.
